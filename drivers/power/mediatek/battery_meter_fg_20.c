...
int get_car_tune_value(void)
{
	...
	static int car_tune_val = 118; // AP measured
#else // hinoki
...
#define DIFFERENCE_SWOCV_RTC		30	/* 30% difference */
#else // hinoki and redwood
...
void get_profile_id_and_volt(int *batt_id, int *batt_id_volt)
{
	...
	*batt_id_volt = 0;
	...
	{
#if defined(CONFIG_BUILD_TEAK) //teak
		bm_print(BMLOG_INFO_LEVEL, "[get_profile_id_and_volt]CEI(K)=> id_volt = %d, swocv_rtc= %d\n", id_volt, batt_meter_cust_data.difference_swocv_rtc);
#else //hinoki and redwood
