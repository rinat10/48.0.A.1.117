...
struct bq25896_info {
	...
	struct mutex pe_access_lock; //CEI comments, add pe_access_lock for bq25896_check_backboost
	bool err_state;
	u32 input_current;
...
#endif
	int irq;//CEI comments, workaround for BQ25896 power drop
};
...
static int bq_charger_set_pep_current_pattern(struct mtk_charger_info *mchr_info, void *data)
{
	...
	mutex_lock(&info->pe_access_lock);//CEI comments, add pe_access_lock for bq25896_check_backboost
	battery_log(BAT_LOG_CRTI, "%s: pump up = %d\n", __func__, up);
	ret = bq25896_pumpx_up(info, up);
	mutex_unlock(&info->pe_access_lock);//CEI comments, add pe_access_lock for bq25896_check_backboost
...
//CEI comments start, add pe_access_lock for bq25896_check_backboost
...
out:
	...
	return ret;
//CEI comments end, add pe_access_lock for bq25896_check_backboost
}
...
static int bq25896_check_backboost(struct bq25896_info *info)
{
	...
	kal_bool current_sign;
        enum bq25896_charging_status chg_stat = BQ25896_CHG_STATUS_NOT_CHARGING;//CEI comments, disable bq25896_check_backboost when charge termination done

	mutex_lock(&info->pe_access_lock);//CEI comments, add pe_access_lock for bq25896_check_backboost
	current_sign = battery_meter_get_battery_current_sign();
        ret = bq25896_get_charging_status(info, &chg_stat);//CEI comments, disable bq25896_check_backboost when charge termination done


	/* battery discharging && charger exist */
	if (current_sign == KAL_FALSE && BMT_status.charger_exist == KAL_TRUE && 
		chg_stat != BQ25896_CHG_STATUS_DONE)//CEI comments, disable bq25896_check_backboost when charge termination done
	...
	const struct i2c_device_id *dev_id)
{
	...
	mutex_init(&info->pe_access_lock);//CEI comments, add pe_access_lock for bq25896_check_backboost
...
err_nodev:
	...
	mutex_destroy(&info->pe_access_lock);//CEI comments , add pe_access_lock for bq25896_check_backboost
