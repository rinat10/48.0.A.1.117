...
#define RT9466_DRV_VERSION "1.0.6_CEI"
...
static int rt9466_toggle_cfo(struct rt9466_info *info)
{
	...
	mutex_lock(&info->i2c_access_lock);
	ret = rt9466_device_read(info->i2c, RT9466_REG_CHG_CTRL2, 1, &data);
	if (ret < 0) {
		battery_log(BAT_LOG_CRTI, "%s read cfo fail(%d)\n", __func__,
			ret);
		goto out;
	}
	...
	ret = rt9466_device_write(info->i2c, RT9466_REG_CHG_CTRL2, 1, &data);
	if (ret < 0) {
		battery_log(BAT_LOG_CRTI, "%s cfo off fail(%d)\n", __func__,
			ret);
		goto out;
	}
	...
	ret = rt9466_device_write(info->i2c, RT9466_REG_CHG_CTRL2, 1, &data);
	if (ret < 0)
		battery_log(BAT_LOG_CRTI, "%s cfo on fail(%d)\n", __func__,
			ret);
out:
	mutex_unlock(&info->i2c_access_lock);
	return 0;
 ...
 * (2) Add 500ms thread to prevent backboost
 *
 * 1.0.5
