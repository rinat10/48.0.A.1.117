...
void tas2555_enable(struct tas2555_priv *pTAS2555, bool bEnable)
{
	...
	if (bEnable) {
		if (!pTAS2555->mbPowerUp) {
			...
			dev_err(pTAS2555->dev, "Enable: load startup sequence\n");
			...
			if (pTAS2555->mpFirmware->mpConfigurations) {
				...
				if (pTAS2555->mbLoadConfigurationPostPowerUp) {
					dev_err(pTAS2555->dev,	"Enable: load configuration: %s, %s\n",
					...
					if (pTAS2555->mpCalFirmware->mnCalibrations) {
						dev_err(pTAS2555->dev, "Enable: load calibration\n");
			...
			dev_err(pTAS2555->dev, "Enable: load unmute sequence\n");
			tas2555_dev_load_data(pTAS2555, p_tas2555_unmute_data);
			pTAS2555->mbPowerUp = true;
		}
	} else {
		if (pTAS2555->mbPowerUp) {
			dev_err(pTAS2555->dev, "Enable: load shutdown sequence\n");
...
int tas2555_set_sampling_rate(struct tas2555_priv *pTAS2555, unsigned int nSamplingRate)
{
	...
	dev_err(pTAS2555->dev, "tas2555_setup_clocks: nSamplingRate = %d [Hz]\n",
		nSamplingRate);
	...
	if (pConfiguration->mnSamplingRate == nSamplingRate) {
		dev_err(pTAS2555->dev, "Sampling rate for current configuration matches: %d\n",
			nSamplingRate);
		return 0;
	}
...
{
	dev_err(pTAS2555->dev, "FW Size        = %d", pFirmware->mnFWSize);
	dev_err(pTAS2555->dev, "Checksum       = 0x%04X", pFirmware->mnChecksum);
	dev_err(pTAS2555->dev, "PPC Version    = 0x%04X", pFirmware->mnPPCVersion);
	dev_err(pTAS2555->dev, "FW  Version    = 0x%04X", pFirmware->mnFWVersion);
	if(pFirmware->mnPPCVersion >= PPC_WITH_DRIVER_VERSION)
	dev_err(pTAS2555->dev, "Driver Version = 0x%04X", pFirmware->mnDriverVersion);
	dev_err(pTAS2555->dev, "Timestamp      = %d", pFirmware->mnTimeStamp);
	dev_err(pTAS2555->dev, "DDC Name       = %s", pFirmware->mpDDCName);
	dev_err(pTAS2555->dev, "Description    = %s", pFirmware->mpDescription);
}
...
static void tas2555_load_block(struct tas2555_priv *pTAS2555, TBlock * pBlock)
{
	...
	dev_err(pTAS2555->dev, "TAS2555 load block: Type = %d, commands = %d\n",
	...
	unsigned int nConfiguration, bool bLoadSame)
{
	...
	dev_err(pTAS2555->dev, "tas2555_load_configuration: %d\n", nConfiguration);
	...
	if ((nConfiguration == pTAS2555->mnCurrentConfiguration) && (!bLoadSame)) {
		dev_err(pTAS2555->dev, "Configuration %d is already loaded\n",
			nConfiguration);
		return;
	}
	...
	if (pTAS2555->mbPowerUp) {
		if (pNewConfiguration->mnPLL != pCurrentConfiguration->mnPLL) {
			...
			dev_err(pTAS2555->dev, "TAS2555: load new PLL: %s, block data\n",
				pNewPLL->mpName);
			...
			dev_err(pTAS2555->dev, "TAS2555: power up TAS2555\n");
			tas2555_dev_load_data(pTAS2555, p_tas2555_startup_data);
			dev_err(pTAS2555->dev,
			...
			tas2555_load_data(pTAS2555, &(pNewConfiguration->mData),
				TAS2555_BLOCK_CONF_COEFF);
			dev_err(pTAS2555->dev, "TAS2555: unmute TAS2555\n");
			tas2555_dev_load_data(pTAS2555, p_tas2555_unmute_data);
		} else {
			dev_err(pTAS2555->dev,
	...
	char *pFileName)
{
	...
	dev_err(pTAS2555->dev, "TAS2555 calibration file = %s, handle = %d\n",
		pFileName, nFile);
	...
	dev_err(pTAS2555->dev, "TAS2555 calibration file size = %d\n", nSize);
	...
	dev_err(pTAS2555->dev, "TAS2555 calibration: %d calibrations\n",
...
void tas2555_fw_ready(const struct firmware *pFW, void *pContext)
{
	...
	if (pTAS2555->mpFirmware->mpConfigurations){
		...
		dev_err(pTAS2555->dev, "clear current firmware\n");
	...
	unsigned int nProgram)
{
	...
	while (!bFound 
		...
		if (pTAS2555->mpFirmware->mpConfigurations[nConfiguration].mnProgram 
			...
			if(nSampleRate == 0){
				bFound = true;
				dev_err(pTAS2555->dev, "find default configuration %d\n", nConfiguration);
			}else if(nSampleRate 
				== pTAS2555->mpFirmware->mpConfigurations[nConfiguration].mnSamplingRate){
				bFound = true;
				dev_err(pTAS2555->dev, "find matching configuration %d\n", nConfiguration);
			}else{
				nConfiguration++;
			}
		}else{
	...
	dev_err(pTAS2555->dev, "load program %d\n", nProgram);
	...
	dev_err(pTAS2555->dev, "uCDSP Checksum: 0x%02x\n", Value);
	nResult = pTAS2555->read(pTAS2555, TAS2555_PLL_CLKIN_REG, &Value);
	dev_err(pTAS2555->dev, "TAS2555 PLL_CLKIN = 0x%02X\n", Value);
	p_tas2555_startup_data[TAS2555_STARTUP_DATA_PLL_CLKIN_INDEX] = Value;

	if (pTAS2555->mbPowerUp){
		dev_err(pTAS2555->dev, "device powered up, load startup\n");
		tas2555_dev_load_data(pTAS2555, p_tas2555_startup_data);
		dev_err(pTAS2555->dev, 
	...
	int nCalibration)
{
	...
	{
		dev_err(pTAS2555->dev, "load new calibration file %s\n", TAS2555_CAL_NAME); 	
		tas2555_load_calibration(pTAS2555, TAS2555_CAL_NAME);
		nCalibration = 0;
	}
