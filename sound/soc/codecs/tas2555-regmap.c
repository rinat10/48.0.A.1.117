...
/*	
	dev_err(pTAS2555->dev, "%s: BOOK:PAGE:REG %u:%u:%u\n", __func__,
	...
	const struct i2c_device_id *pID)
{
	...
	unsigned int n;
	int nResult;
	int i2c_read_times = 0;

	dev_err(&pClient->dev, "%s enter TAS2555 addr 0x(%x)\n", __FUNCTION__, pClient->addr);
	...
	// mt_set_gpio_pull_enable(115, GPIO_PULL_ENABLE);
	// mt_set_gpio_pull_select(115, GPIO_PULL_UP);
	mt_set_gpio_dir(115, GPIO_DIR_OUT);
	mt_set_gpio_out(115, GPIO_OUT_ONE);
	// udelay(1000);
	dev_err(&pClient->dev, "%s set gpio 115 as high\n", __FUNCTION__);

#ifdef ENABLE_GPIO_RESET
	pTAS2555->reset_gpio =
		of_get_named_gpio(pClient->dev.of_node, "ti,reset-gpio", 0);
	dev_err(&pClient->dev, "reset gpio is %d\n", pTAS2555->reset_gpio);
	...
	do {
		...
		dev_err(&pClient->dev, "I2C retry-TAS2555: %d %d\n",nResult, i2c_read_times);
		i2c_read_times ++;
	} while ((nResult < 0) && (i2c_read_times < 100));
	if(nResult < 0){
